...
function dcNavigateWithTelemetry(location, uri, eventName, eventProperties) {
  if(window.recordEvt && window.persistEvt) {
    window.recordEvt(eventName, eventProperties);  
    window.persistEvt();
  } 
  dcNav(location, uri, 'NEW');
}
